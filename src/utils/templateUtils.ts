/**
 * Template utilities compatibility layer
 * 
 * This file provides backward compatibility for existing imports while 
 * forwarding to the new modular template utility files.
 * 
...
if (process.env.NODE_ENV === 'development') {
  console.warn(
    'src/utils/templateUtils.ts is deprecated and will be removed in a future version. ' +
    'Please update imports to use the new modular structure from src/utils/template/* instead.'
  );
}
...
export const checkForCommonTemplateErrors = (code: string): string | null => {
  ...
  const dataProps = ['workExperience', 'education', 'skills', 'projects'];
 ...
 */
export const validateTemplateCode = (code: string): { isValid: boolean; error?: string } => {
  try {
    // Check if it's empty
    if (!code || !code.trim()) {
 ...
 */
const checkForMissingKeys = (code: string): string | null => {
  // Look for .map() calls
 ...
 */
const checkForInvalidIndexUsage = (code: string): string | null => {
  // Identify array mapping contexts
  const mapContexts: {start: number, end: number}[] = [];
  let depth = 0;
  let mapStart = -1;
